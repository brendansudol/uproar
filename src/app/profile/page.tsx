...
async function getProfileData(userId: string): Promise<{
  ...
  stats: { averageRating: number | null; totalSubmissions: number; activeDays: number }
}> {
  const supabase = await createClient()
  ...
  const { data: favoritesData, error: favoritesError } = await supabase
    .from("favorites")
    .select(
      "created_at, submission:submissions(id, setup, punchline, analysis, createdAt:created_at)",
    )
    .eq("user_id", userId)
    .order("created_at", { ascending: false })
